<template>
    <div class="max-w-[1440px] mx-auto pl-[120px] pr-28 ">
        <div class="relative">
            <div class="bg-littleBlue w-[4px] h-[56px] absolute top-[170px] -left-[calc(100%-400px)] right-0 mx-auto"></div>
            <div class="bg-littleBlue w-[4px] h-[56px] absolute top-0 left-0 right-0 mx-auto"></div>
            <div class="bg-littleBlue w-[4px] h-[56px] absolute top-[170px] left-[1180px] right-0 mx-auto"></div>
        </div>
        <div class="flex items-center justify-between ">
            <div class="mb-16 flex flex-col relative">
                <img class="absolute w-[600px] h-[600px] top-[-191px] left-[-123px]" src="/src/assets/img/Header/bgHeader.png" alt="">
                <h1 class="text-darkBlue max-w-[448px]  mb-[33px] font-bold  text-[56px] leading-[59px]">Matnni ovozga o'girish</h1>
                <p class="text-darkBlue max-w-[510px] mb-7">21-oktabr, O'zbek tiliga davlat tili maqomi
                    berilganligining 31 yillik munosabati bilan
                    o'zbekcha matnni nutqga o'girish
                    texnologiyasini ishlab chiqdik</p>
                <div class="flex">
                    <div class="flex flex-col  mr-14">
                        <div class="flex items-center mb-3">
                            <img class="mr-4" src="/src/assets/icons/Header/voice-search1.svg" alt="">
                            <span>Tabiiy ovozda</span>
                        </div>
                        <div class="flex items-center">
                            <img class="mr-4" src="/src/assets/icons/Header/deadline.svg" alt="">
                            <span>Yuqori tezlik</span>
                        </div>
                    </div>
                    <div class="flex flex-col">
                        <div class="flex items-center mb-3 ">
                            <img class="mr-4" src="/src/assets/icons/Header/artificial-intelligence.svg" alt="">
                            <span>To'liq sun'iy intelekt asosida</span>
                        </div>

                        <div class="flex items-center">
                            <img class="mr-4" src="/src/assets/icons/Header/voice.svg" alt="">
                            <span>2ta ovozda tinglash imkoniyati</span>
                        </div>
                    </div>
                </div>
            </div>
            <div>
                <img class="w-[600px] h-[600px]" src="/src/assets/img/Header/robot.png" alt="">
            </div>
        </div>
    </div>
</template>